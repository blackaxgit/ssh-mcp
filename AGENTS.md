...
**`execute`, `upload` and `download` in `ssh.py` are thin tracing wrappers — the behaviour lives in `_execute_impl` / `_upload_impl` / `_download_impl`.** If you are told to change what `execute()` does, edit `_execute_impl`; the public method only opens an OTel span and delegates. Editing the wrapper is the obvious wrong turn here. OpenTelemetry is optional in effect, not in installation: the `otel` extra is gone — `opentelemetry-api` is now a hard dependency of `mcp>=2`, so the tracing API is always importable and `server.py::_span` / `ssh.py::_ssh_tracer` always create spans. What stays optional is whether anything is recorded: with no SDK and exporter installed, those spans are no-ops. `opentelemetry-sdk` remains a dev dependency for the in-memory exporter in `tests/test_otel.py`. **Spans are a leak surface — treat them like logs.** This now covers the SDK's own middleware spans too, not just ssh-mcp's: they record `ssh.command_length`, never the command itself, and error text is `_redact_secrets`'d and truncated to 200 characters. Do not add an attribute carrying a command, a path, or raw error output.
...
**Every push to `main` publishes a container image.** The `docker` job's `PUBLISH` is `github.event_name == 'push' && (github.ref == 'refs/heads/main' || startsWith(github.ref, 'refs/tags/v'))`, so a merged PR pushes `latest` + `type=sha` tags to GHCR with **no environment gate and no human approval** — a tag is not the only path to a publish. PRs build and scan only.

**The image is multi-arch (`linux/amd64,linux/arm64`) as of 0.8.1, and that makes the Trivy gate arity load-bearing.** A multi-arch push produces an *index*, and Trivy resolves an index to the runner's own platform, so **one** scan step gates amd64 and publishes arm64 uninspected — with a green gate. There is therefore one scan step per platform, selected via `TRIVY_PLATFORM` (trivy-action v0.36.0 exposes no `platform` input but passes environment through). For the same reason the digest-verification step **derives** its expected manifest set from the scanned reference instead of comparing against `steps.build.outputs.digest`, which is now an index digest whose children are what `imagetools create` copies into the tag — hardcoding it would fail every publish run, the same shape as the 2026-07-26 incident one level down. `test_published_image_is_multi_arch_and_every_platform_is_scanned` asserts platforms-built equals platforms-scanned.

**The PR path builds amd64 only, so arm64 build breakage surfaces on `main`, not on the PR.** `load: true` cannot load a multi-platform result, so the PR build stays single-platform for fast feedback and Trivy has a local image to scan. The consequence is asymmetric and deliberate: a Dockerfile change that breaks the arm64 leg passes review and turns `main` red. That is loud and publishes nothing — the digest push happens in the same failed step — but it does mean an arch-sensitive change should be built locally first (`docker buildx build --platform linux/arm64 .`), which is how 0.8.1's arm64 leg was verified before merge. Both pinned bases were checked to be multi-arch indexes at the same time (`python:3.14-slim-trixie` and `ghcr.io/astral-sh/uv:0.12.10`); pinning a *single-platform* digest there is the failure this would otherwise find late.
...
**Every job in every workflow declares `timeout-minutes`, and the suite asserts it.** Omitting it inherits GitHub's 360-minute default, so one hung step holds a runner for six hours — which 0.8.0 hit for real: a test that dropped `O_NONBLOCK` did not fail, it *hung* opening a writer-less fifo. Limits are ≥10× the measured 14-69 s job durations, so they cannot trip a healthy-but-slow run. `test_every_job_declares_a_timeout` also rejects a non-positive value, because `timeout-minutes: 0` is schema-valid and would fail every job instantly. Note the limit does **not** consume the `pypi` reviewer-approval wait — a job's clock starts after approval, verified against the v0.7.0 run (16 s runtime behind a 7-minute wait) — so `publish-pypi: 10` cannot time out while waiting for a human.
...
- **`{REDACTED+ELIDED}` in a log means the audit record is incomplete, and the false positives are deliberate.** Redaction replaces from the credential marker to the end of the whitespace-delimited token — which is what stops a partially quoted secret leaking — but shell separators are not whitespace, so `echo --password=X;reboot` logged as `echo --password={REDACTED}` and the `reboot` was **absent from the record while the command ran** (reproduced against a live host, 0.8.1). Narrowing the replacement to the separator was rejected: a secret containing `;` would then have its tail printed. So the value stays fully hidden and `_placeholder_for` flags the record instead. It errs toward flagging — a secret that merely *contains* a `;` is flagged too — because telling those apart needs shell parsing this module refuses to do. Do not "fix" the noise by narrowing the replacement, and do not add a separator to `_SHELL_SEPARATOR_RE` that cannot survive `_split_ws_preserving` (newline and CR were removed for exactly that reason).
...
- **`SSH_MCP_HTTP_TOKEN_FILE` follows symlinks ON PURPOSE — do not "harden" it with `O_NOFOLLOW`.** `server.py::_read_token_file` validates what it reads and does so by `fstat`-ing the descriptor it then reads, so there is no stat-then-open TOCTOU window. It REFUSES a non-regular file, a file over `_MAX_TOKEN_FILE_BYTES` (checked on the `st_size` snapshot *and* on bytes actually read, because a file can grow between `fstat` and `read`), a group/other **writable** mode, and non-UTF-8 content. It only WARNS on a group/other **readable** mode and on an owner that is neither this process nor root. That refuse/warn split is deliberate and load-bearing: Docker Swarm mounts secrets `0444`, Kubernetes defaults to `0644`, and both are commonly root-owned, so making *readability* or *ownership* fatal would break the two most common secret mounts outright — whereas neither carries a group/other **write** bit, so refusing writability costs nothing and closes an integrity hole (whoever can write the file picks the token that authorises remote command execution). Unlike `paths.py::ensure_root`, it deliberately does NOT pass `O_NOFOLLOW`: Kubernetes projects every secret key as a symlink chain (`key` -> `..data/key` -> `..<timestamp>/key`) so kubelet can swap it atomically, so refusing symlinks would reject every Kubernetes secret mount. The confinement argument that justifies `O_NOFOLLOW` for `transfer_root` does not transfer — there is no tree beneath a token file to confine. `tests/test_http_transport.py::TestTokenFileValidation` pins every one of these, including a test that fails specifically if someone adds the flag. Note `O_NONBLOCK` is NOT claimed as a liveness guarantee: it stops a fifo hanging startup, but a regular file on a hostile FUSE mount can still block, which is accepted as a local DoS by whoever already controls the config.

- **The token-file permission checks are POSIX-gated, and that gate is load-bearing.** On Windows CPython *synthesises* `st_mode` as `S_IFREG|0o666` for any non-read-only file, so the group/other-writable refusal would reject **every** Windows token file and advise `chmod go-w`; `os.O_NONBLOCK` and `os.geteuid` do not exist there at all, and `AttributeError` is neither `OSError` nor `UnicodeDecodeError`, so it would escape both handlers as a raw traceback. 0.7.0 read this file with `Path.read_text()`, which worked on Windows, so all three were regressions introduced by the 0.8.0 fix itself and caught by review, not by CI — the matrix in `ci.yml` is ubuntu-only. `tests/test_http_transport.py::TestTokenFileValidation::test_posix_only_apis_are_guarded_for_windows` simulates all three conditions at once; do not delete it in the belief that "we only ship Linux containers", because the PyPI wheel has no OS classifier.

- **A token source that is configured but yields nothing ABORTS; that is not the same as unset.** Until 0.8.0 an empty `SSH_MCP_HTTP_TOKEN_FILE` (or a whitespace-only `SSH_MCP_HTTP_TOKEN`) flowed through `token = raw_token or None` and the bearer middleware was simply never attached, so a loopback bind served the RCE-capable endpoint with no authentication while the operator believed the mounted secret was protecting it. Both now raise at startup. A literal empty string is still "unset", and configuring **no** token source at all still permits unauthenticated loopback — that is the documented single-user workstation mode and `test_unset_token_still_allows_unauthenticated_loopback` pins it, so do not "simplify" the three cases back into one truthiness check.

- **`uv sync` must use `--locked` in CI, and lint tools must run via `uv run`, never `uvx`.** `uvx` resolves the newest release at run time: ruff 0.16.0 widened its default rule set from 59 to 413 rules and turned CI red on commits that had passed days earlier. `[tool.ruff.lint] select` therefore pins the rule set explicitly — widening it is a deliberate decision, not a cleanup. It is currently `["E4", "E7", "E9", "F", "I", "BLE"]`: 0.8.0 measured the 413-rule default (70 findings, dominated by tests-only `SIM115`/`SIM117` cosmetics) and rejected it, adopting only import sorting and blind-except. **`BLE` is load-bearing, not style** — the 9 broad `except Exception` blocks each carry a `# noqa: BLE001` naming why, so a *new* blind except fails CI in a codebase where a swallowed exception can silently drop a security control. Do not strip those `noqa`s to "clean up", and do not enable `RUF100`: the reasoning is in `pyproject.toml`. `tests/test_ci_lint_determinism.py` enforces the `uvx`/`--locked` half of this.
...
- **`server.py` chains the MCP session-manager lifespan via the SDK's public `MCPServer.session_manager`.** An earlier version reached into Starlette's private `router.lifespan_context`; do not go back to it.
