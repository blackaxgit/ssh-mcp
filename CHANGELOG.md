...
The marker is applied by **every** rule that replaces to the end of a whitespace-delimited run, and not by the two that are bounded on both sides (URL basic-auth, terminated by `@`; the quoted `-p'…'` form, terminated by its closing quote) — those cannot swallow a following command, so flagging them would be a pure false positive. Both halves of that were found by CI's 200-example Hypothesis profile on the first push, not locally at 50: a value was flagged under `--password=` and not under `--token=` because the enumerated-env-var rule matched the second one first, and separately a later rule re-matched an already-redacted flag and *downgraded* `{REDACTED+ELIDED}` back to the plain placeholder. An inconsistent marker is worse than none, since a reader takes its absence as proof nothing was cut, so an existing marker is now sticky. `AGENTS.md` records the process fix: run `HYPOTHESIS_PROFILE=ci` locally before pushing a redaction change.
